const API = 'https://raw.githubusercontent.com/Dissaving/git-dis/master/JSON';
...
class ProductsList extends List {
    constructor(cart, container = '.products', url = "/catalogDataLesson3.json") {
        super(url, container);
        this.cart = cart;
        this.getJson()
...
class Cart extends List {
    constructor(container = ".cart-block", url = "/getBasketLesson3.json") {
        super(url, container);
        this.getJson()
            .then(data => {
     ...
     */
    addProduct(element) {
        this.getJson(`${API}/addToBasketLesson3.json`)
            .then(data => {
                if (data.result === 1) {
    ...
    removeProduct(element) {
        this.getJson(`${API}/deleteFromBasketLesson3.json`)
            .then(data => {
                if (data.result === 1) {
                    let productId = +element.dataset['id'];
                    let find = this.allProducts.find(product => product.id_product === productId);
